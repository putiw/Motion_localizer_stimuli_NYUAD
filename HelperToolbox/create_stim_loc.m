...
elseif strcmp(whichLoc, 'fst')    
    ...
    for dir = 1:2
            ...
            for c = 1:length(pa.coherence) % for each coherence
                ...
                for s = 1:pa.numberOfRepeats % create a stimulus for each block
                    ...
                    for nn = 1:pa.numFlips  % Loop through all the frames for this trial
                        ...
                        dotMatrix.(char(whichLoc))(ap,c,:,:,nn,s) = dot_mat;
        ...
        if dir ==1
            ...
            if strcmp(pa.conditionNames{1}, 'cd4')
                dotMatrix.(char(whichLoc)) = flip(dotMatrix.(char(whichLoc)),5);
            end
            
            dotMatrix.(char(whichLoc)) = cat(3,tmp.(char(whichLoc)),dotMatrix.(char(whichLoc)));
            
        end
        
        dotMatrix.cd0 = dotMatrix.(char(whichLoc));
    ...
    for ii = 1:size(pa.design,1)
        ...
        if pa.trial(5) ==1
            pa.current_condition = char(whichLoc);
        else
